       ...
       DeconToolsV2.Peaks.clsPeak[] mspeakList, ResultCollection resultList)
        {
            ...
            foreach (DeconToolsV2.HornTransform.clsHornTransformResults hornResult in transformResults)
            {
                ...
                GetIsotopicProfile(saveMemoryByAddingMinimalPeaksToIsotopicProfile, 
                    hornResult.marr_isotope_peak_indices, mspeakList, ref profile);
        ...
        private void GetIsotopicProfile(bool storeOnlyMonoAndMostAbundant, int[] peakIndexList, DeconToolsV2.Peaks.clsPeak[] peakdata, ref IsotopicProfile profile)
        {
            ...
            if (storeOnlyMonoAndMostAbundant)     //only store two of the peaks... to reduce memory usage. 
            {
                DeconToolsV2.Peaks.clsPeak mostIntenseDeconPeak = deconMonopeak;

                for (int i = 1; i < peakIndexList.Length; i++)     //start with second peak and find most intense peak and add it to profile
                {
                    DeconToolsV2.Peaks.clsPeak deconPeak = lookupPeak(peakIndexList[i], peakdata);
                    if (deconPeak.mdbl_intensity > mostIntenseDeconPeak.mdbl_intensity)
                    {
                        mostIntenseDeconPeak = deconPeak;
                    }
                }
                MSPeak mostIntensePeak = convertDeconPeakToMSPeak(mostIntenseDeconPeak);
                profile.Peaklist.Add(mostIntensePeak);
            }
            else
            {
                ...
                {
                    DeconToolsV2.Peaks.clsPeak deconPeak = lookupPeak(peakIndexList[i], peakdata);
                    profile.Peaklist.Add(convertDeconPeakToMSPeak(deconPeak));
                }
